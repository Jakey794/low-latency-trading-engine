//! `simulate` — runtime replay with strategy, risk config, and portfolio summary.
...
pub struct SimulateArgs {
    ...
    /// Built-in strategy: `market-maker` / `market_making` or `momentum`.
    #[arg(long)]
    pub strategy: String,
    ...
    /// Optional max order quantity risk limit (overrides risk-config).
    #[arg(long)]
    pub max_order_qty: Option<u64>,

    /// JSON risk limits file (see `data/config/risk_demo.json`).
    ...
    #[arg(long, conflicts_with_all = ["output", "portfolio_summary"])]
    pub summary_only: bool,

    /// Print final portfolio snapshot as JSON to stderr.
    #[arg(long, alias = "portfolio")]
    pub portfolio_summary: bool,
}

pub fn run(args: SimulateArgs) -> Result<()> {
    let strategy_name = normalize_strategy_name(&args.strategy);
    let strategy = create_builtin(strategy_name, args.strategy_id)
    ...
    let mut risk_limits = match &args.risk_config {
        Some(path) => load_risk_limits(path)?,
        None => RiskLimits::default(),
    };
    if let Some(max_qty) = args.max_order_qty {
        risk_limits.max_order_qty = Some(max_qty);
    }
    ...
    let result = runtime
        ...
        .with_context(|| format!("failed to simulate {}", args.path.display()))?;
...
pub fn load_risk_limits(path: &Path) -> Result<RiskLimits> {
    ...
    let limits: RiskLimits = serde_json::from_reader(BufReader::new(file))
        .with_context(|| format!("failed to parse risk config {}", path.display()))?;
    Ok(limits)
}

fn normalize_strategy_name(name: &str) -> &str {
    match name {
        "market-maker" | "market_maker" | "mm" => "market_making",
